...
class FeatureContext extends MinkContext implements Context
{
     ...
     *  soafter each other they will return an error if the animation/loading has not finished.
     * @param int $milisecs
   ...
   public function searchOnRET($searchCriteria)
   {
    ...
    public function journeyPlanner(TableNode $departureAndArrival)
    {
        ...
        for($i = 0; $i < count($departures); $i++)  {
            ...
            $this->fillField('tx_retjourneyplanner_form[search][arrival][name]', $arrivals[$i]);
            $this->pressButton("Nu bekijken");
        }
    }
}